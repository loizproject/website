...
const { data: page, error } = await useAsyncData("about", async () => {
  const query = `
    query {
      page(id: "/refund-policy", idType: URI) {
        id
        title
        content
        featuredImage {
...
const meta = {
  title:
    "Refund Policy: Loiz Tours & Travels Ltd - Your Gateway to Exhilarating & Seamless Travel Experiences!",
  description:
...
<template>
  <div id="refund-policy">
    ...
    <div
      ...
      :style="`background-image: linear-gradient(to right,#ffffff, #ffffff01), url(${
