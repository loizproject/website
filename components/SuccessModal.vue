...
const emit = defineEmits(["close", "resendEvent"]);
...
<template>
  <v-dialog persistent v-model="modal" width="85%" max-width="400px">
    ...
    >
      ...
      <p v-if="message.text" class="py-4">
        {{ message.text }}
      </p>
      <div v-if="message.content" class="py-4 success-content">
        <p v-for="(n, index) in message.content" :key="index">
          {{ n }}
        </p>
      </div>
      ...
      <nuxt-link v-if="login" to="/auth/login">
        <v-btn class="submit">Sign in</v-btn>
      </nuxt-link>
    </v-card>
  </v-dialog>
</template>
